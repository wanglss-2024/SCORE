...
// [[Rcpp::export]]
Rcpp::List nlopt_optimize_fixedc(
    ...
    // Optimize
    auto objective_and_grad = [&metadata, &O, &X, &Y, &w, &C](const double * params, double * grad) -> double {
...
// [[Rcpp::export]]
Rcpp::List nlopt_optimize_vestep_fixedc(
        const Rcpp::List & data  , // List(Y, X, O, w)
        const Rcpp::List & params, // List(M, S, C)
    ...
    const auto init_M = Rcpp::as<arma::mat>(params["M"]); // (n,q)
    const auto init_S = Rcpp::as<arma::mat>(params["S"]); // (n,q)
    const auto  C = Rcpp::as<arma::mat>(params["C"]); // covinv (p,q)
