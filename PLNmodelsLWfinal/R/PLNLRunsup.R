#' Poisson lognormal model with low rank (unsupervised)
#'
#' Fit the Poisson lognormal (low rank) with a variational algorithm. Use the (g)lm syntax for model specification (covariates, offsets).
#'
...
#'
#' @return an R6 object with class [PLNLRunsupfit()]
#'
...
#' @seealso The class [`PLNLRunsupfit`]
#' @importFrom stats model.frame model.matrix model.response model.offset
#' @export
PLNLRunsup <- function(formula, data, subset, weights, control = PLN_param()) {
  ...
  ## Initialize model
  myPLNLRunsup <- PLNLRunsupfit$new(args$Y, args$X, args$O, args$w, args$formula, control)
  
  ## Compute the group means
  if (control$trace > 0) cat("\n Fitting model...")
  myPLNLRunsup$optimize(args$Y, args$X, args$O, args$w, control$config_optim)
  # 
  if (control$trace > 0) cat("\n DONE!\n")
  myPLNLRunsup
}
