#' Poisson lognormal model with fixed V (unsupervised)
#'
...
#'
#' @return an R6 object with class [PLNfixedVunsupfit()]
#'
...
#'
#' @rdname PLNfixedVunsup
#' @examples
#' data(trichoptera)
#' trichoptera <- prepare_data(trichoptera$Abundance, trichoptera$Covariate)
#' myPLNLDA <- PLNLDA(Abundance ~ 1, grouping = Group, data = trichoptera)
#' @seealso The class [`PLNfixedVunsupfit`]
#' @importFrom stats model.frame model.matrix model.response model.offset
#' @export
PLNfixedVunsup <- function(formula, data, subset, weights, control = PLN_param()) {
  ...
  # Initialize model
  myPLNfixedVunsup <- PLNfixedVunsupfit$new(args$Y, args$X, args$O, args$w, args$formula, control)
  
  ## Compute the group means
  if (control$trace > 0) cat("\n Fitting model...")
  myPLNfixedVunsup$optimize(args$Y, args$X, args$O, args$w, control$config_optim)
