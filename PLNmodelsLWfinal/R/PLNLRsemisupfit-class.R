## %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
##  CLASS PLNLRsemisupfit
## %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%

#' An R6 Class to represent a PLNfit in a mixture framework
#'
#' @description The function \code{\link{PLNLRsemisup}} produces a collection of models which are instances of object with class \code{PLNmixturefit}.
#'
...
#' @seealso The function \code{\link{PLNLRsemisup}}
PLNLRsemisupfit <-
  R6Class(classname = "PLNLRsemisupfit",
          inherit = PLNfit_lowrank,
          ## %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
          ## PRIVATE MEMBERS
          ## %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
          private = list(
          ...
          public  = list(
            ...
            initialize = function(grouping, responses, covariates, offsets, weights, formula, control) {
              ...
              private$C <- control$config_optim$C0
              private$B <- control$config_optim$B0
              private$M  <- matrix(0, n, private$q)
              private$S  <- matrix(1, n, private$q)
              
              private$optimizer$main <- nlopt_optimize_rank_unsup
              private$optimizer$vestep <- nlopt_optimize_vestep_rank_unsup
            },
            
            #' @description Update a [`PLNmixturefit`] object
            update = function(B=NA, Sigma=NA, Omega=NA, C=NA, M=NA, S=NA, Z0=NA, Z1=NA, A0=NA, A1=NA, Ji0=NA, Ji1=NA, R2=NA, monitoring=NA) {
              private$B <- B
              private$Sigma <- Sigma
              private$Omega <- Omega
              private$C <- C
            ...
            #' @description Optimize a [`PLNLRsemisupfit`] model
            ...
            optimize = function(responses, covariates, offsets, weights, config) {
              ...
              while (!cond) {
                ...
                args <- list(data   = list(Y = responses, X0 = X0, X1 = X1, O = offsets, w = weights, w0 = private$tau[,1], w1 = private$tau[,2]),
                             params = list(B = private$B, M = private$M, S = private$S, C = private$C),
                             config = config)
            ...
            predict = function(newdata,
              ...
              args <- extract_model(call("PLNLRsemisup", formula = private$formula, data = newdata), envir)
          ...
          active = list(
            ...
            model_par  = function() {list(B = private$B, Sigma = private$Sigma, Omega = private$Omega, Theta = t(private$B), C = private$C, Pi = self$mixtureParam)},
