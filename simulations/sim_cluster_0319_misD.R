...
B = cbind(rep(0, q), 0.2, rep(0.8, q), rep(0.2, q))
U = log(rpois(n+N, 2)*10 + 1)
group = rbinom(n+N, 1, 0.4)
X = cbind(1, U, group, rbinom(n+N, 1, 0.4)); colnames(X) = c("X1", "X2", "X3", "X4")
...
B.fixedv.sup = sqrt(sum((pln.fixedv.sup$model_par$B[-1,] - t(B[,-c(1,4)]))^2) / sum(B[,-c(1,4)]^2))
...
B.fixedv.unsup = sqrt(sum((pln.fixedv.unsup$model_par$B[-1,] - t(B[,-c(1,4)]))^2) / sum(B[,-c(1,4)]^2))
...
B.fixedv.semisup = sqrt(sum((pln.fixedv.semisup$model_par$B[-1,] - t(B[,-c(1,4)]))^2) / sum(B[,-c(1,4)]^2))
...
save(B.fixedv.sup, L.fixedv.sup, AUC.fixedv.sup, PRAUC.fixedv.sup, cos.fixedv.sup,
     ...
     file=paste0("results_0319_new/misDweak_N", N, "_n", n, "_p", p, "_q", q, "_b", b, "_type", type ,".RData"))
