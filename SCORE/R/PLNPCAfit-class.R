...
PLNPCAfit <- R6Class(
    ...
    public  = list(
      ...
      initialize = function(rank, grouping, responses, covariates, offsets, weights, formula, control) {
        ...
        private$B  <- matrix(0, ncol(covariates)+1, rank)
      },
      ...
      project = function(newdata, 
        ...
        for (k in 1:K) { ## One VE-step to estimate the conditional (variational) likelihood of each group
          ...
          ve_step_list[[k]] <- self$optimize_vestep(X, args$O, args$Y, args$w, control = control)
      ...
      show = function() {
        ...
        cat("    plot.PLNPCAfit(), predict.PLNPCAfit()\n")
      }
