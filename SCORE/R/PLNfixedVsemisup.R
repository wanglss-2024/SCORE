#' Poisson lognormal model with fixed V (semisupervised)
#'
#' Fit the Poisson lognormal (fixed V) with a variational algorithm. Use the (g)lm syntax for model specification (covariates, offsets).
#'
...
#'
#' @return an R6 object with class [PLNfixedVsemisupfit()]
#'
...
#'
#' @rdname PLNfixedVsemisup
#' @examples
#' data(trichoptera)
#' trichoptera <- prepare_data(trichoptera$Abundance, trichoptera$Covariate)
#' myPLNLDA <- PLNLDA(Abundance ~ 1, grouping = Group, data = trichoptera)
#' @seealso The class [`PLNfixedVsemisupfit`]
#' @importFrom stats model.frame model.matrix model.response model.offset
#' @export
PLNfixedVsemisup <- function(formula, data, subset, weights, grouping, control = PLN_param()) {
  ...
  # Initialize model
  myPLNfixedVsemisup <- PLNfixedVsemisupfit$new(grouping, args$Y, args$X, args$O, args$w, args$formula, control)
  
  ## Compute the group means
  if (control$trace > 0) cat("\n Fitting model...")
  myPLNfixedVsemisup$optimize(args$Y, args$X, args$O, args$w, control$config_optim)
