...
Rcpp::List nlopt_optimize_rank(
    ...
    auto objective_and_grad = [&metadata, &O, &X, &Y, &w](const double * params, double * grad) -> double {
        ...
        metadata.map<C_ID>(grad) = (diagmat(w) * (A - Y)).t() * (X * B + M) + (A.t() * (S2.each_col() % w)) % C;
...
// [[Rcpp::export]]
Rcpp::List nlopt_optimize_mstep_rank(
        const Rcpp::List & data  , // List(Y, X, O, w)
        const Rcpp::List & params, // List(B, C)
    ...
    const auto metadata = tuple_metadata(init_B, init_C);
    enum { B_ID, C_ID }; // Names for metadata indexes
    ...
    // Optimize
    auto objective_and_grad = [&metadata, &O, &X, &Y, &w, &M, &S](const double * params, double * grad) -> double {
        const arma::mat B = metadata.map<B_ID>(params);
        const arma::mat C = metadata.map<C_ID>(params);
