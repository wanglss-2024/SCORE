...
void set_lower_bounds(NloptStruct * opt, const std::vector<double> & lb) {
    if(lb.size() != nlopt_get_dimension(opt)) {
        throw Rcpp::exception("set_lower_bounds: parameter size mismatch");
    }
    if(nlopt_set_lower_bounds(opt, lb.data()) != NLOPT_SUCCESS) {
        throw Rcpp::exception("nlopt_set_lower_bounds");
    }
}
